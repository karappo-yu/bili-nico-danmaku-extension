...
B 站(ビリビリ)の Web プレイヤーでニコニコ弾幕をそのまま再生できる Chrome 拡張機能(MV3)。

ニコニコの弾幕ファイル(JSON/XML)を選ぶだけで、ニコニコ風の弾幕が B 站の動画に重なって表示されます。タイムラインのずれはオフセット調整で合わせられます。

[中文](README.md) | [English](README.en.md) | **日本語**

## スクリーンショット

![安達としまむら・ニコニコ弾幕が画面いっぱいに](png/nico-danmaku-1.png)

![DECO*27「マシュマロ」・2500 件の弾幕を読み込み](png/nico-danmaku-2.png)

## インストール

1. Chrome → `chrome://extensions` →「デベロッパー モード」をオン
2.「パッケージ化されていない拡張機能を読み込む」→ このディレクトリ(`manifest.json` がある階層)を選択
3. 任意の B 站 動画/番組ページを開くと、右上に「Nico 弾幕」パネルが表示されます

## 使い方

1.「📂 弾幕ファイルを選択」で弾幕ファイルを読み込み
2. 動画を再生すると弾幕が滑らかに同期します(rAF dt 累積クロック、動画のフレームレートに依存しない 60fps 均一な移動)
3. B 站の動画とニコニコ本家のタイムラインがずれている場合は「オフセット」スライダーで調整(正=弾幕が早い、負=遅い)
4. フォントサイズ/不透明度は即時反映;B 站 本来の弾幕はプレイヤー標準の弾幕スイッチで制御
...
- 同じ動画を開き直す/リロードすると**弾幕とオフセットを自動復元** — ファイルを選び直す必要なし
- **エピソード/パート切り替えで対応する弾幕を自動ロード**:1 話の弾幕を 2 話に切り替えると 2 話の弾幕が読み込まれ、戻すときも同様
-「弾幕をクリア」= 現在の動画との関連付けを解除(次回は自動ロードされない。他の話には影響なし)
...
| nico XML | 生の `<packet>` / `<chat>` 弾幕 XML |
...
- `test-data/sample-nico.json` — v1 形式のテスト用弾幕 68 件
...
- 弾幕同期: rAF dt 累積クロック(nico-comment-dl preview と同じ方式)——再生/シーク/復帰時に `video.currentTime` へ再アンカーし、その後はメディアフレーム単位の補正なしで自由動作(メディアクロックのジッターは補正すると逆に微細なガタつきを生むため)。浮動小数点 vpos を毎フレーム `drawCanvas` に渡す。シーク差 >1.5s で `clear()` して再描画。一時停止中は最終フレームを保持し、シーク時のみ再描画
...
- 弾幕クロックは再生/シーク/バッファ復帰時に動画へ再アンカー。バッファリング中(`waiting`)は凍結し、`playing` で再調整
- 全画面表示時、パネルは fullscreen 要素の外にあるため非表示になります(弾幕自体はプレイヤーと一緒に全画面表示されます)
- CA 弾幕の `@jump` は無視(ブラウザ環境に複数動画切り替えの意味がないため)
- B 站 本来の弾幕には関与しない:プレイヤー標準の弾幕スイッチで制御

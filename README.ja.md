...
5.「コメント表示」スイッチ:ニコニココメントの表示/非表示を一時切替(動画とファイルの関連付けは保持。「コメントをクリア」とは別)
6. **ニコニコかんたんコメント**:sm 番号または URL(例 `sm9` / `https://www.nicovideo.jp/watch/sm9`)を入力 → 該当動画のかんたんコメントを取得し**ローカルにキャッシュ**。ボタンは状態で変化:新しい番号 →「かんたんコメントをダウンロード」、ダウンロード済み →「再ダウンロード」(キャッシュを強制更新)。ブラウザでニコニコにログイン済みならログイン状態(`user_session`)を自動使用
7. **ニコニコ動画IDの自動マッピング**(初期オフ):マッピング表に該当がある動画を開くと、対応するニコニココメントを自動ダウンロードし、パネルに N 站のタイトルを表示して確認できます。マッピングデータはまだ少ないため、必要な時に手動でオン

### マッピング表

- **共有表**: [mappings.json](mappings.json)(GitHub リポジトリ。拡張機能に同梱 + リモート差分マージ、10 分キャッシュ)、**手動メンテナンス** — マッピング追加はファイルを編集してコミット
- 作品ごとに分類(シーズン/シリーズのメタデータ付き、多対 1 対応):
  ```json
  {
    "shows": {
      "作品名": {
        ...
        "nicoSeries": 497560,
        "eps": {
          "ep1231523": { "sm": "so44461978", "bTitle": "第1話 ...", "nTitle": "..." }
        }
      }
    },
    "videos": {
  ...
  ```
- 動画を開く → 表にヒット → 対応コメントを自動ダウンロード(「ニコニコ動画IDの自動マッピング」スイッチがオンの場合)。ヒットしなければ sm 番号を手動入力
...
- `content.js` — プレイヤー検出、overlay マウント、ファイル読み込み、rAF 同期ループ、オフセット/フォント/不透明度、マッピング表の参照/自動ダウンロード、SPA 再マウント(MutationObserver)。パネルは動画ページ(/video/、/bangumi/play/、/list/)でのみ作成
- `content.css` — パネル UI
- `background.js` — MV3 service worker:ニコニコ API リクエストのプロキシ(CORS 制約の回避、ログイン状態付きも可)とマッピング表の取得(raw.githubusercontent + jsDelivr フォールバック)
...
- ニコニココメントのダウンロード:ブラウザでニコニコにログイン済みならログイン状態(`user_session`)を自動使用。未ログイン時は公開コメント(かんたんコメント)のみ取得可能

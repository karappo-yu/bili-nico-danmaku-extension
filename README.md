...
3. 打开任意 B 站视频/番剧页,播放器控制条出现「N」按钮(全屏按钮旁)

## 使用

1. 点控制条「N」按钮 → 显示/隐藏菜单(默认隐藏,点面板外自动关闭)
2. 点「📂 选择弹幕文件」载入弹幕文件
3. 播放视频,弹幕随视频平滑同步(rAF dt 累积时钟,60fps 均匀步进,不受视频帧率限制)
...
8. **自动映射**:打开有映射的视频时,自动下载对应 nico 弹幕(bvid↔sm 映射表,本地记录 + 远程共享表,面板显示 B 站↔N 站标题对供核对)。「自动映射」开关可关

### 映射表

- **本地映射**:每次下载 nico 弹幕时自动记录 bvid → sm + 双标题,查表优先用本地
- **远程共享表**:[mappings.json](mappings.json)(GitHub 仓库,24h 缓存),多对 1(B 站重复搬运同一视频),贡献方式:PR 添加 `"BVxxx": { "sm": "sm9", "bTitle": "...", "nTitle": "..." }`
- 查表顺序:本地记录 → 远程表;都没有则手动输入 sm 号
...
- niconico 弹幕下载:浏览器已登录 niconico 时自动使用登录态(`user_session`);未登录只能获取公开弹幕(精选)
